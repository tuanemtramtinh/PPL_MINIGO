...
//function
func_declaration: FUNC (method_declaration | ) ID LEFT_PAREN list_func_arguments_prime RIGHT_PAREN ((all_types | list_array_specific ID) | ) NEWLINE* LEFT_CURLY (NEWLINE | list_statement)* RIGHT_CURLY;
...
//struct
struct_declaration: TYPE ID STRUCT LEFT_CURLY (NEWLINE | list_struct_argument)* RIGHT_CURLY terminate;
list_struct_argument: struct_argument list_struct_argument | struct_argument; 
struct_argument: ID all_types terminate;

//interface
interface_declaration: TYPE ID INTERFACE LEFT_CURLY (NEWLINE | interface_method_declaration)* RIGHT_CURLY terminate;
...
statement: const_declaration_stmt | var_declaration_stmt | assignment_statement | if_statement | for_statement | break_statement | continue_statement | function_call | return_statement;
...
//assignment
assignment_statement: list_assignment_lhs (ASSIGN_COLON | ADD_ASSIGN | SUB_ASSIGN | MUL_ASSIGN | DIV_ASSIGN | MOD_ASSIGN) expression (SEMICOLON | NEWLINE*);
...
//if
if_statement: IF LEFT_PAREN expression RIGHT_PAREN NEWLINE* LEFT_CURLY (NEWLINE | list_statement)* RIGHT_CURLY NEWLINE* list_elseif_prime NEWLINE* ((ELSE NEWLINE* LEFT_CURLY (NEWLINE |list_statement)* RIGHT_CURLY) | ) terminate;
list_elseif_prime: list_elseif | ;
list_elseif: elseif list_elseif | elseif;
elseif: ELSE IF LEFT_PAREN expression RIGHT_PAREN NEWLINE* LEFT_CURLY (NEWLINE | list_statement)* RIGHT_CURLY;

//for
for_statement: (basic_for | init_for | range_for) NEWLINE* LEFT_CURLY (NEWLINE | list_statement)* RIGHT_CURLY terminate;
...
//call
// call_statement: (ID'.')?ID LEFT_PAREN list_expression_prime RIGHT_PAREN terminate;
function_call: ((list_assignment_lhs DOT) | )ID LEFT_PAREN list_expression_prime RIGHT_PAREN (terminate | );
...
expression6: expression6 (LEFT_SQUARE expression RIGHT_SQUARE | DOT expression) | expression7;
